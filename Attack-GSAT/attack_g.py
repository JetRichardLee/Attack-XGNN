...
from ExplanationEvaluation.explainers.GSAT import GSAT
from ExplanationEvaluation.explainers.GSATAttacker import Deduction_GSATAttacker,Loss_GSATAttacker
...
for indx in  range(0,40,1):
    ...
    N=features.shape[1]
    extime_s = time.time()
    explainer = GSAT(model, graphs, features, task,epochs=50)
    ...
    if attack=="deduction" or attack == "loss":
        ...
        if attack == "deduction":
            attacker=Deduction_GSATAttacker(model, graphs, features, task,beta=beta,N=N,r_epochs=10)
        else:
            attacker=Loss_GSATAttacker(model, graphs, features, task,gamma=gamma,r_epochs=10)
        ...
        if attack == "deduction":
            attacker=Deduction_GSATAttacker(model, r_graphs, features, task,beta=beta,N=N,r_epochs=2)
        else:
            attacker=Loss_GSATAttacker(model, r_graphs, features, task,gamma=gamma,r_epochs=10)
