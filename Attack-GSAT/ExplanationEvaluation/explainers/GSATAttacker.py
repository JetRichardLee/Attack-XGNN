...
class Deduction_GSATAttacker(BaseExplainer):
    """
    A class encaptulating our deduction_based attacker against the GSAT .
    
    
    Original GSAT parameters
    ...
    def gsat_loss(self, masked_pred, original_pred, mask, reg_coefs,epoch):
        """
    ...
    def D_loss(self, masks,mask_preds, original_pred, mask, reg_coefs,epoch):
        """
        ...
        loss = -self.gsat_loss(mask_preds[0], original_pred, masks[0], reg_coefs,epoch)*self.N
        for i in range(1,len(masks)):
            loss+= self.gsat_loss(mask_preds[i], original_pred, masks[i], reg_coefs,epoch)    
            
        return loss
    
    def A_loss(self, masks,mask_preds, original_pred, mask, reg_coefs,epoch):
        """
    ...
    def learn_deletion(self, indices=None,fliter =None, bias=None):
        ...
        for e in tqdm(range(0, self.r_epochs)):
            ...
            for n in indices:
                ...
                id_loss = self.D_loss(masks, preds,  torch.argmax(original_pred).unsqueeze(0), mask, self.reg_coefs,e)
    ...
    def learn_addition(self, indices=None,fliter =None, bias=None):
        ...
        for e in tqdm(range(0, self.r_epochs)):
            ...
            for n in indices:
                ...
                id_loss = self.A_loss(masks, preds, torch.argmax(original_pred).unsqueeze(0), mask, self.reg_coefs,e)
...
class Loss_GSATAttacker(BaseExplainer):
    """
    A class encaptulating our loss_based attacker against the GSAT .
    ...
    def D_loss(self, masked_pred_gamma, original_pred, mask_gamma, reg_coefs,epoch):
        """
        ...
        r = self.get_r((self.decay_interval), (self.decay_r), epoch, final_r=(self.final_r))
        info_loss = (mask_gamma * torch.log(mask_gamma / r + 1e-06) + (1 - mask_gamma) * torch.log((1 - mask_gamma) / (1 - r + 1e-06) + 1e-06)).mean()

        cce_loss =  torch.nn.functional.cross_entropy(masked_pred_gamma, original_pred)
        return cce_loss + info_loss

    def A_loss(self, masked_pred_gamma, original_pred, mask_gamma, reg_coefs,epoch):
        """
    ...
    def learn_deletion(self, indices=None,fliter =None, bias=None):
        ...
        for e in tqdm(range(0, self.r_epochs)):
            ...
            for n in indices:
                ...
                id_loss = self.D_loss(masked_pred_gamma, torch.argmax(original_pred).unsqueeze(0), mask_gamma, self.reg_coefs,e)
                loss += id_loss
    ...
    def learn_addition(self, indices=None,fliter =None, bias=None):
        ...
        for e in tqdm(range(0, self.r_epochs)):
            ...
            for n in indices:
                ...
                id_loss = self.A_loss(masked_pred_gamma, torch.argmax(original_pred).unsqueeze(0), mask_gamma, self.reg_coefs,e)
                loss += id_loss
