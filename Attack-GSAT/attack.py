...
for indx in test_indices:
    ...
    explainer = GSAT(model, graphs, features, task)
    ...
    if attack=="deduction" or attack == "loss":
        ...
        if attack == "deduction":
            attacker=Deduction_GSATAttacker(model, graphs, features, task,beta=beta)
        else:
            attacker=Loss_GSATAttacker(model, graphs, features, task,gamma=gamma)
        ...
        if attack == "deduction":
            attacker=Deduction_GSATAttacker(model, r_graphs, features, task,beta=beta)
        else:
            attacker=Loss_GSATAttacker(model, r_graphs, features, task,gamma=gamma)
