...
def log_graph(
    ...
    edge_colors = [np.float64(w) for (u, v, w) in Gc.edges.data("weight", default=1)]
...
def read_ogb_products(max_nodes=None, edge_labels=False):
    """ Read data from https://ls11-www.cs.tu-dortmund.de/staff/morris/graphkerneldatasets
        graph index starts with 1 in file
    ...
    for u in G.nodes():
        ...
        if cnt>=1000:
            ...
            G.nodes[u]["feat"] = features[u - 1]
    G.remove_nodes_from(dele)
    labels = labels[:1000]
    G.graph["feat_dim"] = features[0].shape[0]
