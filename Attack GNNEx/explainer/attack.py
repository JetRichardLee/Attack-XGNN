...
class Explainer:
    ...
    # Main method
    def explain(
        self, node_idx, graph_idx=0,fliter=None, bias=None, graph_mode=False, unconstrained=False, model="exp"
    ):
        ...
        if model == "grad":
            ...
            for epoch in range(self.args.num_epochs):
                ...
                ypred, adj_atts = explainer(node_idx_new, fliter = None,bias=None,unconstrained=unconstrained)
    ...
    def against_cold(
        self, node_idx, graph_idx=0,fliter=None,bias=None,
                attack_type="deduction",gamma=None,beta=None,N=None,
                graph_mode=False, unconstrained=False, model="exp",epochs=30
    ):
        """Explain a single node prediction
        """
        # index of the query node in the new adj
        ...
        sub_adj = np.expand_dims(sub_adj, axis=0)
        sub_feat = np.expand_dims(sub_feat, axis=0)
        sub_adj = np.ones(sub_adj.shape)-sub_adj-np.eye(sub_adj.shape[0],sub_adj.shape[1])
        adj   = torch.tensor(sub_adj, dtype=torch.float)
        x     = torch.tensor(sub_feat, requires_grad=True, dtype=torch.float)
        label = torch.tensor(sub_label, dtype=torch.long)
        ...
        if attack_type == "deduction":
            attacker.train()
            begin_time = time.time()
            betas=[0]
            for b in range(N):
                betas.append(b/(N-1)*(1-beta)+beta)
            for epoch in range(epochs):
                attacker.zero_grad()
                attacker.optimizer.zero_grad()
                masks,reses,adj_atts = attacker(node_idx_new, fliter = fliter,bias=bias,bg=betas)
                loss = attacker.dedu_loss(masks,reses, pred_label, node_idx_new, epoch,"cold")
               
                loss.backward()

                attacker.optimizer.step()
                if attacker.scheduler is not None:
                    attacker.scheduler.step()

                mask_density = attacker.mask_density()
                if self.print_training:
                    print(
                        "epoch: ",
                        epoch,
            ...
            masked_adj = (
                    attacker.masked_adj[0].cpu().detach().numpy() * sub_adj.squeeze()
                )
        else:
            attacker.train()
            begin_time = time.time()
            ...
            for epoch in range(epochs):
                attacker.zero_grad()
                attacker.optimizer.zero_grad()
                masks,reses,adj_atts = attacker(node_idx_new, fliter = fliter,bias=bias,bg=gammas)
                loss = attacker.loss_loss(masks,reses, pred_label, node_idx_new, epoch,"cold")
    ...
    def against_hot(
        ...
        if attack_type == "deduction":
            ...
            for epoch in range(epochs):
                ...
                attacker.optimizer.zero_grad()
                masks,reses,adj_atts = attacker(node_idx_new, fliter = fliter,bias=bias,bg=betas)
                loss = attacker.dedu_loss(masks,reses, pred_label, node_idx_new, epoch,"hot")
            ...
            for epoch in range(epochs):
                ...
                attacker.optimizer.zero_grad()
                masks,reses,adj_atts = attacker(node_idx_new, fliter = fliter,bias=bias,bg=gammas)
                loss = attacker.loss_loss(masks,reses, pred_label, node_idx_new, epoch,"hot")
    ...
    def explain_nodes_gnn_stats(self, node_indices, args, graph_idx=0, model="exp"):
        ...
        for i, idx in enumerate(node_indices):
            ...
            if masked_adjs[i].shape[0]<=new_idx+5:
                return None
    ...
    #random
    def explain_nodes_under_random(self, node_indices, original_mask, args, graph_idx=0, top_k=6, p_budget=5, model="exp"):
        """
        explain node under the random attacks
        ...
        original_mask = np.array(original_mask[0])
        o_id = np.argpartition((original_mask+original_mask.T).reshape(-1), -top_k*2)[-top_k*2:]
        

        for node_idx in node_indices:
            
            self.model.eval()
            ypred, adj_att = self.model(torch.FloatTensor(self.feat), torch.FloatTensor(self.adj))
            node_pred = ypred[graph_idx, node_idx, :].detach()
            res_0 = nn.Softmax(dim=0)(node_pred)
            predict_before = np.argmax(np.array(res_0.detach()),axis=0)
            
            _, sub_adj, _, _, neighbors = self.extract_neighborhood(
                node_idx, graph_idx
            )
            degree = np.sum(sub_adj,axis=0)
            for k in range(len(o_id)):
                a1 = o_id[k]//original_mask.shape[0]
                a2 = o_id[k]%original_mask.shape[0]
            for j in range(test_times):
                newd=degree.copy()
                fliter = np.ones(original_mask.shape)
                bias = np.zeros(original_mask.shape)
                p_adj = np.asarray(self.adj).copy()
                cnt =0 
                tomare =0 
                while cnt<p_budget:
                    if tomare>100:#when no edge to perturb
                        break
                    a1 = random.randint(0,original_mask.shape[0]-2)
                    a2 = random.randint(a1+1,original_mask.shape[0]-1)
                    if np.isin([a1*original_mask.shape[0]+a2],o_id)==True or fliter[a1][a2]==0:
                        tomare+=1
                        continue
                    fliter[a1][a2]=fliter[a2][a1]=0
                    if sub_adj[a1][a2]<0.5:
                        bias[a1][a2]=bias[a2][a1]=1
                        p_adj[0,neighbors[a1],neighbors[a2]]=1
                        newd[a1]+=1
                        newd[a2]+=1
                    else:
                        bias[a1][a2]=bias[a2][a1]=0
                        p_adj[0,neighbors[a1],neighbors[a2]]=0
                        newd[a1]-=1
                        newd[a2]-=1
                    cnt+=1
                    
                ls = likelyhood(degree,newd,5)
                self.model.eval()
                ypred, adj_att = self.model(torch.FloatTensor(self.feat), torch.FloatTensor(p_adj))
                node_pred = ypred[graph_idx, node_idx, :].detach()
                res = nn.Softmax(dim=0)(node_pred)
                predict_after = np.argmax(np.array(res.detach()),axis=0)

                if ls>0.000157 or predict_before!=predict_after:
                    continue
                
                now_mask = self.explain(node_idx, graph_idx=graph_idx,fliter=fliter,bias=bias, model=model)
                now_id = np.flip(np.argsort((now_mask+now_mask.T).reshape(-1))[-top_k*2:])
                
                for k in range(len(now_id)):
                    if k>=top_k*2:
                        break
                    if np.isin([now_id[k]],o_id)==False :
                        sumfool+=1
                        
        avg_fool = sumfool/test_times
        return avg_fool
    

    
    
    def explain_nodes_under_killhot(self, node_indices, original_mask, args, graph_idx=0,top_k=6, p_budget=5,  model="exp"):
        ...
        o_id = np.argpartition((original_mask+original_mask.T).reshape(-1), -top_k*2)[-top_k*2:]
        hot_id = np.flip(np.argsort((original_mask+original_mask.T).reshape(-1))[-300:])
        original_fliter = np.ones(original_mask.shape).reshape(-1)
        original_bias = np.zeros(original_mask.shape).reshape(-1)
        original_fliter[o_id]=0
        original_bias[o_id]=1
        original_fliter = original_fliter.reshape(original_mask.shape)
        original_bias = original_bias.reshape(original_mask.shape)
        ...
        for node_idx in node_indices:
            ...
            #cold_id = np.argsort((hot_edge+hot_edge.T).reshape(-1))[:60]
            degree = np.sum(sub_adj,axis=0)
            for t in range(0,1):
                newd=degree.copy()
                cnt=0
                fool=0
                fliter = np.ones(original_mask.shape).reshape(-1)
                bias = np.zeros(original_mask.shape).reshape(-1)
                p_adj = np.asarray(self.adj).copy()
                cnt=0
                for k in range(len(hot_id)):
                    if cnt>=p_budget:
                        break
                    if np.isin([hot_id[k]],o_id)==True or fliter[hot_id[k]]==0:
                        continue
                    cnt+=1
                    fliter[hot_id[k]]=0
                    bias[hot_id[k]]=0
                    a1 = hot_id[k]//original_mask.shape[0]
                    a2 = hot_id[k]%original_mask.shape[0]
                    p_adj[0,neighbors[a1],neighbors[a2]]=0
                    newd[a1]-=1
                ls = likelyhood(degree,newd,5)
                self.model.eval()
                ...
                res = nn.Softmax(dim=0)(node_pred)
                predict_after = np.argmax(np.array(res.detach()),axis=0)
                if ls>0.000157 or predict_after!=predict_before:
                    continue
                fliter = fliter.reshape(original_mask.shape)
                bias = bias.reshape(original_mask.shape)
                ...
                final_fool = max(final_fool,fool)
            
        return final_fool
    
    def explain_nodes_under_loss(self, node_indices, original_mask, args, graph_idx=0, model="exp", top_k=6, p_budget=5, gamma=0.7):
        ...
        for node_idx in node_indices:
            ...
            node_idx_new, sub_adj, sub_feat, _, neighbors = self.extract_neighborhood(
                node_idx, graph_idx
            )
            ...
            hot_edge = self.against_hot(node_idx,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,
                                        attack_type="loss",gamma=gamma)
            hot_id = np.flip(np.argsort((hot_edge+hot_edge.T).reshape(-1))[-300:])
            
            cold_edge = self.against_cold(node_idx,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,
                                        attack_type="loss",gamma=gamma)
            cold_id = np.flip(np.argsort((cold_edge+cold_edge.T).reshape(-1))[-300:])
            
            for t in range(0,p_budget+1):
                newd=degree.copy()
                cnt=0
                fliter = np.ones(original_mask.shape)
                bias = np.zeros(original_mask.shape)
                ...
                for k in range(len(cold_id)):
                    if cnt>=2*t:
                        break
                    if np.isin([cold_id[k]],o_id)==True or fliter[cold_id[k]]==0:
                        continue
                    cnt+=1
                    fliter[cold_id[k]]=0
                    bias[cold_id[k]]=1
                    a1 = cold_id[k]//original_mask.shape[0]
                    a2 = cold_id[k]%original_mask.shape[0]
                    p_adj[0,neighbors[a1],neighbors[a2]]=1
                    #adjbias[0,neighbors[a1],neighbors[a2]]=1
                    newd[a1]+=1
                for k in range(len(hot_id)):
                    if cnt>=2*p_budget:
                        break
    ...
    def explain_nodes_under_dedu(self, node_indices, original_mask, args, graph_idx=0, top_k=6, p_budget=5, model="exp",beta=0.7,N=4):
        ...
        for node_idx in node_indices:
            ...
            hot_edge = self.against_hot(node_idx,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,
                                        attack_type="deduction",beta=beta,N=N)
            hot_id = np.flip(np.argsort((hot_edge+hot_edge.T).reshape(-1))[-300:])
            cold_edge = self.against_cold(node_idx,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,
                                        attack_type="deduction",beta=beta,N=N)
            cold_id = np.flip(np.argsort((cold_edge+cold_edge.T).reshape(-1))[-300:])
    ...
    def explain_graph_under_random(self, graph_indices, top_k=5, p_budget=2):
        """
        Explain graphs.
        """
        avgfools = []
        ...
        for graph_idx in graph_indices:
            masked_adj = self.explain(node_idx=0, graph_idx=graph_idx, graph_mode=True)
            o_id = np.argpartition((masked_adj+masked_adj.T).reshape(-1), -top_k*2)[-top_k*2:]
            
            self.model.eval()
            ypred, adj_att = self.model(torch.FloatTensor(self.feat.float()), torch.FloatTensor(self.adj.float()))
            graph_pred = ypred[graph_idx, :].detach()
            res_0 = nn.Softmax(dim=0)(graph_pred)
            predict_before = np.argmax(np.array(res_0.detach()),axis=0)
            
            
            sub_adj = self.adj[graph_idx]
            degree = np.sum(sub_adj.numpy(),axis=0)
            
            for j in range(test_times):
                fliter = np.ones(sub_adj.shape)
                bias = np.zeros(sub_adj.shape)
                newd=degree.copy()
                p_adj = np.asarray(self.adj).copy()
                cnt =0 
                tomare =0 
                while cnt<p_budget:
                    if tomare>100:
                        break
                    a1 = random.randint(0,sub_adj.shape[0]-2)
                    a2 = random.randint(a1+1,sub_adj.shape[0]-1)
                    if np.isin([a1*sub_adj.shape[0]+a2],o_id)==True or fliter[a1][a2]==0:
                        tomare+=1
                        continue
                    fliter[a1][a2]=fliter[a2][a1]=0
                    if sub_adj[a1][a2]<0.5:
                        bias[a1][a2]=bias[a2][a1]=1
                        p_adj[a1][a2]=p_adj[a2][a1]=1
                        
                        newd[a1]+=1
                        newd[a2]+=1
                    else:
                        bias[a1][a2]=bias[a2][a1]=0
                        p_adj[a1][a2]=p_adj[a2][a1]=0
                        newd[a1]-=1
                        newd[a2]-=1
                    cnt+=1
                
                ls = likelyhood(degree,newd,5)
                now_mask = self.explain(node_idx=0, graph_idx=graph_idx,fliter=fliter,bias=bias, graph_mode=True)
                now_id = np.flip(np.argsort((now_mask+now_mask.T).reshape(-1))[-top_k*2:])
                
                self.model.eval()
                ypred, adj_att = self.model(torch.FloatTensor(self.feat.float()), torch.FloatTensor(p_adj))
                graph_pred = ypred[graph_idx, :].detach()
                res_1 = nn.Softmax(dim=0)(graph_pred)
                predict_after = np.argmax(np.array(res_1.detach()),axis=0)
            ...
            sumfool = sumfool/test_times
            avgfools.append(sumfool)
        # plot cmap for graphs' node features

        return avgfools

    def explain_graphs_killhot(self, graph_indices, top_k=5, p_budget=2):
        """
        Explain graphs.
        """
        maxfools = []
        for graph_idx in graph_indices:
            ...
            hot_id = np.flip(np.argsort((masked_adj+masked_adj.T).reshape(-1))[-30:])
            ...
            p_adj = np.asarray(self.adj).copy()
            cnt=0
            fliter = np.ones(masked_adj.shape)
            bias = np.zeros(masked_adj.shape)
            fool=0
            fliter = np.ones(masked_adj.shape).reshape(-1)
            bias = np.zeros(masked_adj.shape).reshape(-1)
            newd=degree.copy()
            for k in range(len(hot_id)):
                if cnt>=p_budget*2:
                    break
                ...
                fliter[hot_id[k]]=0
                bias[hot_id[k]]=0
                a1 = hot_id[k]//masked_adj.shape[0]
                a2 = hot_id[k]%masked_adj.shape[0]
                p_adj[graph_idx,a1,a2]=0
            ...
            fliter = fliter.reshape(sub_adj.shape)
            bias = bias.reshape(sub_adj.shape)
            now_mask = self.explain(node_idx=0, graph_idx=graph_idx,fliter=fliter,bias=bias, graph_mode=True)
            now_id = np.flip(np.argsort((now_mask+now_mask.T).reshape(-1))[-top_k*2:])
            ...
            maxfools.append(final_fool)
        # plot cmap for graphs' node features

        return maxfools
    
    def explain_graphs_under_loss(self, graph_indices, top_k=5, p_budget=2,gamma=0.7):
        """
        Explain graphs.
        """
        maxfools = []
        for graph_idx in graph_indices:
            ...
            original_fliter = np.ones(masked_adj.shape).reshape(-1)
            original_bias = np.zeros(masked_adj.shape).reshape(-1)
            original_fliter[o_id]=0
            original_bias[o_id]=1
            original_fliter = original_fliter.reshape(masked_adj.shape)
            original_bias = original_bias.reshape(masked_adj.shape)
            
            
            hot_edge = self.against_hot(node_idx=0,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,graph_mode=True,
                                        attack_type="loss",gamma=gamma)
            hot_id = np.flip(np.argsort((hot_edge+hot_edge.T).reshape(-1))[-60:])
            cold_edge = self.against_cold(node_idx=0,graph_idx=graph_idx,fliter = original_fliter,bias=original_bias,graph_mode=True,
                                        attack_type="loss",gamma=gamma)
            cold_id = np.flip(np.argsort((cold_edge+cold_edge.T).reshape(-1))[-60:])
            ...
            for t in range(p_budget+1):
                ...
                for k in range(len(cold_id)):
                    ...
                    fliter[cold_id[k]]=0
                    bias[cold_id[k]]=1
                    a1 = cold_id[k]//masked_adj.shape[0]
                    a2 = cold_id[k]%masked_adj.shape[0]
                    p_adj[graph_idx,a1,a2]=1
                for k in range(len(hot_id)):
                    if cnt>=2*p_budget:
                        break
                    if np.isin([hot_id[k]],o_id)==True or fliter[hot_id[k]]==0:
                ...
                fliter = fliter.reshape(sub_adj.shape)
                bias = bias.reshape(sub_adj.shape)
                now_mask = self.explain(node_idx=0, graph_idx=graph_idx,fliter=fliter,bias=bias, graph_mode=True, model="grad")
                now_id = np.flip(np.argsort((now_mask+now_mask.T).reshape(-1))[-top_k*2:])
    ...
    def explain_graphs_under_dedu(self, graph_indices, top_k=5, p_budget=2,beta=0.6,N=4):
        """
        Explain graphs.
        """
        maxfools = []
        for graph_idx in graph_indices:
            ...
            sub_adj = self.adj[graph_idx].float()
            degree = np.sum(sub_adj.numpy(),axis=0)
            final_fool=0
...
class ExplainModule(nn.Module):
    ...
    def _masked_adj(self,fliter=None,bias=None):
        sym_mask = self.mask
        if fliter is None:
            ...
            if self.mask_act == "sigmoid":
                sym_mask = (torch.sigmoid(self.mask)*torch.FloatTensor(fliter)+torch.FloatTensor(bias))
            elif self.mask_act == "ReLU":
                sym_mask = nn.ReLU()(self.mask+torch.FloatTensor(fliter))
    ...
    def forward(self, node_idx,fliter=None, bias=None, unconstrained=False, mask_features=True, marginalize=False):
        ...
        if unconstrained:
            ...
            self.masked_adj = self._masked_adj(fliter,bias)
            if mask_features:
                feat_mask = (
...
class AgainstModule(nn.Module):
    def __init__(
        ...
        super(AgainstModule, self).__init__()
        self.adj = adj
        self.x = x
        self.model = model
        self.label = label
    ...
    def _masked_adj(self,fliter=None,biass=None):
        sym_mask = self.mask
        if fliter is None:
            if self.mask_act == "sigmoid":
                sym_mask = torch.sigmoid(self.mask)
            ...
            if self.mask_act == "sigmoid":
                sym_mask = (torch.sigmoid(self.mask*torch.FloatTensor(fliter))+torch.FloatTensor(biass))
            elif self.mask_act == "ReLU":
                sym_mask = nn.ReLU()(self.mask+torch.FloatTensor(fliter))
    ...
    def forward(self, node_idx,fliter=None,bias=None,bg=[0,0.5], marginalize=False):
        x = self.x.cuda() if self.args.gpu else self.x

        self.masked_adj = self._masked_adj()
        masked_adj_v = []
        for i in range(len(bg)):
            masked_adj_v.append(self._masked_adj(fliter,bg[i]*bias))
            
        reses = [None for i in range(len(bg))]
        adj_atts = [None for i in range(len(bg))]
        losses = [None for i in range(len(bg))]
        for i in range(len(bg)):
            new_pred,adj_atts[i]= self.model(x, masked_adj_v[i])            
            if self.graph_mode:
                reses[i]=nn.Softmax(dim=0)(new_pred[0])
            else:
                reses[i] = nn.Softmax(dim=0)(new_pred[self.graph_idx, node_idx, :])
            
        return masked_adj_v,reses,adj_atts
    
    
    def loss(self, mask, pred, pred_label, node_idx):
        """
        Args:
            pred: prediction made by current model
            pred_label: the label predicted by the original model.
    ...
    def dedu_loss(self, masks, preds, pred_label, node_idx,loss_type,N):
        """
        Args:
            pred: prediction made by current model
            pred_label: the label predicted by the original model.
        ...
        loss = self.loss(masks[0],preds[0],pred_label,node_idx)
        for i in range (len(N)-1):
            loss += self.loss(masks[0],preds[0],pred_label,node_idx)
        for i in range (len(N)):
            loss-=self.loss(masks[i+1],preds[i+1],pred_label,node_idx)
    ...
    def loss_loss(self, mask,pred, pred_label, node_idx, loss_type):
        """
        Args:
            pred: prediction made by current model
            pred_label: the label predicted by the original model.
        ...
        loss = -self.loss(mask, pred, pred_label, node_idx)
        if loss_type == "hot":
            loss =-loss
