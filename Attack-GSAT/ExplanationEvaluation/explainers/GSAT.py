...
class GSAT(BaseExplainer):
    """
    We modify the PGExplainer to replicate the GIBExplainer of GSAT
    ...
    """
    def __init__(self, model_to_explain, graphs, features, task, epochs=100, lr=0.003, temp=(5.0, 2.0), reg_coefs=(0.05, 1.0), sample_bias=0, final_r=0.7, decay_interval=10, decay_r=0.1):
    ...
    def train(self, indices=None):
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for n in indices:
                ...
                id_loss = self._loss(masked_pred, torch.argmax(original_pred).unsqueeze(0), mask, self.reg_coefs, e)
                loss += id_loss
    ...
    def explain(self, index):
        ...
        if self.type == 'node':
            ...
            graph = ptgeom.utils.k_hop_subgraph(index, 2, self.graphs)[1]
            embeds = self.model_to_explain.embedding(self.features, self.graphs).detach()
        else:
        ...
        expl_graph_weights = mask
        return (
         graph, expl_graph_weights)
